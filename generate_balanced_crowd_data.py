...
used_places = set()
place_id_counter = 1
for district in ['Kathmandu', 'Bhaktapur', 'Lalitpur']:
    district_places = [p for p in CURATED_PLACES if p['district'] == district]
    for time_slot in TIME_SLOTS:
        random.shuffle(district_places)
        group_places = {'High': [], 'Medium': [], 'Low': []}
        available_places = [p for p in district_places if p['place_name'] not in used_places]
        for group, count in [('High', 3), ('Medium', 2), ('Low', 2)]:
            group_selected = 0
            for place in available_places:
                if place['place_name'] in used_places:
                    continue
                group_places[group].append(place)
                used_places.add(place['place_name'])
                group_selected += 1
                if group_selected == count:
                    break
            if group_selected < count:
                needed = count - group_selected
                for place in district_places:
                    if place not in group_places[group]:
                        group_places[group].append(place)
                        group_selected += 1
                        if group_selected == count:
                            break
        for group, places in group_places.items():
            for place in places:
...
print('balanced_crowd_data.csv generated with all required columns for model training.') 